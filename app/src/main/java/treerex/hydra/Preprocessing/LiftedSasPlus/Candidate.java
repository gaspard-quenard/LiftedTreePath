...
public class Candidate {
    ...
    public String getUniqueStringRepresentation() {
        ...
        List<Integer> seenCountedVariables = new ArrayList<>();
        List<Integer> seenFixedVariables = new ArrayList<>();
        ...
        for (int i = 0; i < this.mutexGroup.size(); i++) {
            ...
            for (Integer variableIdx : pred.paramsId) {
                ...
                if (variable.isCountedVar && seenCountedVariables.contains(variableIdx)) {
                    candidateString.append(seenCountedVariables.indexOf(variableIdx));
                }
                else if (!variable.isCountedVar && seenFixedVariables.contains(variableIdx)) {
                    candidateString.append(seenFixedVariables.indexOf(variableIdx));
                }
                else {
                    int idInAtomCandidate;
                    if (variable.isCountedVar) {
                        idInAtomCandidate = seenCountedVariables.size();
                        seenCountedVariables.add(variableIdx);
                    } else {
                        idInAtomCandidate = seenFixedVariables.size();
                        seenFixedVariables.add(variableIdx);
                    }

                    variable.idInAtomCandidate = idInAtomCandidate;
                    candidateString.append(idInAtomCandidate);
                }
    ...
    public String getSMTStringRepresentation() {
        return this.getUniqueStringRepresentation().replaceAll("[{}]", "").replaceAll("[ ,:]", "_");
    }
}
